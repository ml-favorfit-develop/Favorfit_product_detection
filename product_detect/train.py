...
def train(epoch, model, criterion, optimizer, train_loader):
    ...
    for idx, (inputs, labels) in tqdm(enumerate(train_loader), leave=False, desc=f"{epoch} train", total=len(train_loader)):
        inputs.to(device)
        labels.to(device)
    ...
    logger.wnb_write({"train_Loss": train_acc, "train_Acc": train_loss})
...
def val(epoch, model, criterion, val_loader):
    ...
    conf_labels=[]; conf_preds=[]
    with torch.no_grad():
        running_loss = 0.0; running_acc = 0.0
        
        for idx, (inputs, labels) in tqdm(enumerate(val_loader), leave=False, desc=f"{epoch} val", total=len(val_loader)):
            inputs.to(device)
            labels.to(device)
            ...
            preds = torch.argmax(outputs, dim=1)
            acc = torch.sum(preds == labels).item() / len(inputs) * 100
    ...
    logger.wnb_write_conf_mat(conf_labels, conf_preds, args.numclasses)
...
def run(args):
    ...
    model = Classifier(base_model=args.model, num_classes=args.numclasses, pretrained=args.pretrained)
    ...
    for epoch in range(args.epochs):
        train(args, epoch, model, criterion, optimizer, train_loader)
