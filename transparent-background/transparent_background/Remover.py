...
if __name__ == "__main__":
    ...
    base_path = "images/*"
    scale_path_list = [glob(os.path.join(scale_dir, "*")) for scale_dir in glob(base_path)]

    jit_device_state = [(True, "cuda"), (False, "cuda"),
                        (True, "cpu"), (False, "cpu")]
    
    for jit_avail, device in jit_device_state:
        remover = Remover(fast=True, jit=jit_avail, device=device)
        model = remover.model
        ...
        result_json = []
        for file_list in scale_path_list:
            scale = os.path.split(os.path.split(file_list[0])[0])[1]
            print(f"-----{scale}-----")
            for file_path in file_list:
                ...
                Image.fromarray(output).save(f"results/{scale}/{file_name}_{jit_avail}_{device}.png")
                
                del img; del output

            resources_dict = monitor.print_and_get_results()
            model_resources_dict = print_and_get_flops_parameter(model=remover.model, input_x=x)
            resources_dict.update(model_resources_dict)
            resources_dict["scale"] = scale
            result_json.append(resources_dict)

            monitor.reset(); del x
            time.sleep(3)
            print()

        with open(f"results/{jit_avail}_{device}_monitoring.json", mode="w", encoding="utf-8") as f:
            json.dump(result_json, f, indent=4, ensure_ascii=False)
        
        monitor.kill()
        torch.cuda.empty_cache()
        del monitor; del model; del remover; del transform
